...
/** Cache key used by GET /api/products — invalidated on every stock-changing mutation. */
...
function reserveIdempotencyKey(idempotencyKey: string) {
  return `idempotency:reserve:${idempotencyKey}`;
}

function confirmIdempotencyKey(reservationId: string) {
  return `idempotency:confirm:${reservationId}`;
}
...
export async function createReservation(
  ...
  if (idempotencyKey) {
    const cached = await redis.get<string>(reserveIdempotencyKey(idempotencyKey));
    if (cached) {
  ...
  if (idempotencyKey) {
    await redis.set(
      ...
      serializeReservationCache(reservation),
      { ex: IDEMPOTENCY_CACHE_TTL_SECONDS },
    );
  }

  // Invalidate products cache — available stock has changed
...
export async function confirmReservation(
  ...
  const cached = await redis.get<string>(confirmIdempotencyKey(reservationId));
  if (cached) {
    return deserializeReservation(
      typeof cached === "string" ? cached : JSON.stringify(cached),
    );
  ...
  await redis.set(
    confirmIdempotencyKey(reservationId),
    serializeReservationCache(reservation),
    { ex: IDEMPOTENCY_CACHE_TTL_SECONDS },
  );

  // Invalidate products cache — stock was decremented on confirm
  await redis.del(PRODUCTS_CACHE_KEY);
...
export async function releaseReservation(
  ...
  // Invalidate products cache — reserved stock was freed
  await redis.del(PRODUCTS_CACHE_KEY);
